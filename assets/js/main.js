...
function GetName() {
    ...
    //textField.isContentEditable = true;
    //textField.focus();
    ...
    textField.addEventListener('keydown', function (event) {
        ...
        if (event.key === 'Enter') {
            // Trigger something when Enter is pressed
            textField.style.opacity = '0';
            setTimeout(function() {
                var greeting = document.getElementById('greeting');
                greeting.remove();
            }, 1000);
            moveSymbolsToTheTopLeft();
        }
    });
}

function moveSymbolsToTheTopLeft(){
    ...
    var oldSymbolContainers = document.getElementsByClassName('old-symbol-container');
    // Move symbol containers to the top left
    for (var i = 0; i < oldSymbolContainers.length; i++) {
        var oldSymbolContainer = oldSymbolContainers[i];
        // Add a class for fading out
        oldSymbolContainer.style.marginLeft = '0';
    }
    // Move greeting wrapper to the top
    var greetingWrapper = document.getElementById("greeting-wrapper");
    greetingWrapper.style.top = '0';
    greetingWrapper.style.height = '15%';

    // Move greeting reflection to the top
    var greetingReflection = document.getElementById("greeting-reflection");
    greetingReflection.style.height = '100%';
    greetingReflection.style.position = 'absolute';
    
    function processSymbolContainers() {
        var greetingReflections = greetingWrapper.children;
        console.log("greeting reflections: " + greetingReflections.length);
        for(var i = 0; i < greetingReflections.length; i++) {
            var reflection = greetingReflections[i];
            reflection.style.opacity = '1';
            CreateSymbolContainers(reflection, 30, 3, 3);
            var symbolContainers = reflection.getElementsByClassName('new-symbol-container');
            const delayBetweenImages = 2; // milliseconds
            let totalDelay = 0;
        
            for (let j = 0; j < symbolContainers.length; j++) {
                for (let i = 0; i < enteredName.length; i++) {
                    (function (index) {
                        const symbolContainer = symbolContainers[index];
                        symbolContainer.style.marginLeft = '0';
        
                        setTimeout(function () {
                            loadImageAndAddToContainer(enteredName[i], symbolContainer, index);
                        }, totalDelay);
        
                        totalDelay += delayBetweenImages;
                    })(j);
                }
            }
        }
    ...
    setTimeout(processSymbolContainers, 1000);
    setTimeout(function() {
